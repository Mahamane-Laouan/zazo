            ...
            <div class="auth-wrapper-form">
                ...
                <form class="" action="{{route('login_post')}}" method="post" id="form-id">
                    ...
                    @if(isset($recaptcha) && $recaptcha['status'] == 1)
                        ...
                        @else
                        <div class="col-6 bg-white rounded d-flex">
                            @if($custome_recaptcha)
                                <img src="{{ $custome_recaptcha->inline() }}" class="rounded w-100" />
                            @else
                                <div class="rounded w-100 text-center py-3 bg-light">
                                    <strong>{{ session('six_captcha') }}</strong>
                                </div>
                            @endif
                            <div class="p-3 pr-0 capcha-spin reloadCaptcha">
                                <i class="tio-cached"></i>
